...
DB_CONFIG = None
# Using 'logger' as the variable name for the logger instance
logger = logging.getLogger(__name__) # Standard practice for module logger
if not logger.hasHandlers():
    handler = logging.StreamHandler()
    handler.setLevel(logging.INFO) 
    formatter = logging.Formatter('%(asctime)s - %(name)s - %(levelname)s - [API] %(message)s')
...
if STATIC_DIR.exists() and STATIC_DIR.is_dir():
    ...
    logger.warning(f"Static directory {STATIC_DIR} not found. Static files might not be served.")

if TEMPLATES_DIR.exists() and TEMPLATES_DIR.is_dir():
    templates = Jinja2Templates(directory=TEMPLATES_DIR)
else:
    logger.error(f"Templates directory {TEMPLATES_DIR} not found. HTML templates will not be served.")
    templates = None # type: ignore

@app.get("/predictions/latest_for_table")
async def get_latest_predictions_for_table_from_db():
    if not DB_CONFIG: raise HTTPException(status_code=503, detail="Database service unavailable.")
    try:
        predictions = get_latest_prediction_for_all_tickers(DB_CONFIG)
        return {"status": "success", "data": predictions or []}
    except Exception as e:
        logger.error(f"API Error fetching latest predictions for table: {e}", exc_info=True)
...
async def get_historical_context_for_prediction(ticker: str):
    ...
    if latest_pred_info and latest_pred_info.get("target_prediction_date"):
        ...
        logger.info(f"Predicted price for ref date {prediction_ref_date_str} for {ticker_upper}: {predicted_price_for_ref} (type: {type(predicted_price_for_ref)})")
        ...
        if not df_hist_from_db.empty:
            ...
            logger.info(f"Found {len(historical_actual_dates_list)} historical points for {ticker_upper} from DB for chart.")
        else:
            logger.warning(f"No historical data found in raw_stock_data for {ticker_upper} for the chart.")
    else:
