...
def train_final_model(
    ...
    best_params: dict,
    X_train: np.ndarray,
    y_train: np.ndarray,  # Expect NumPy arrays
    X_test: Optional[np.ndarray],
    y_test: Optional[np.ndarray],  # Expect NumPy arrays or None
    num_features: int,
    num_stocks: int,
    y_scalers: list,
    tickers: list,
...
def run_training(config_path: str, run_id_arg: str) -> Optional[str]:
    ...
    try:
        ...
        logger.info(
            f"Model training using dataset from run_id: {current_dataset_run_id}"
        )
        ...
        X_train_scaled = load_scaled_features(
            db_config, current_dataset_run_id, "X_train"
        )
        y_train_scaled = load_scaled_features(
            db_config, current_dataset_run_id, "y_train"
        )
        X_test_scaled = load_scaled_features(
            db_config, current_dataset_run_id, "X_test"
        )
        y_test_scaled = load_scaled_features(
            db_config, current_dataset_run_id, "y_test"
        )
        ...
        (
            ...
            tickers=tickers,
            device=device,
            training_epochs=final_training_epochs,
        ...
        try:
            ...
            registered_model_name = (
                mlflow_experiment_name  # Consistent with registration
            )
            ...
            try:
                for stock_idx, ticker_name in enumerate(tickers):
                    ...
                    if test_predictions_np.shape[0] > 0:
                        ...
                        pred_idx_to_save = (
                            0  # Save the first prediction if pred_len > 1
                        )

                        last_pred_on_test_for_ticker_scaled = test_predictions_np[
                            -1, pred_idx_to_save, stock_idx
                        ]
                        last_pred_on_test_for_ticker_inv = y_scalers[
                            stock_idx
                        ].inverse_transform(
        ...
        logger.error(
            f"Error in run_training for dataset_run_id {run_id_arg}: {e}", exc_info=True
        )
        return None
...
if __name__ == "__main__":
    ...
    try:
        ...
        Exception
    ) as e_main:  # Catching other potential errors during setup or teardown
        logger.error(
