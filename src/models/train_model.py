...
except ImportError: # Fallback for direct execution or different environment
    from model_definitions import StockLSTM, StockLSTMWithAttention, StockLSTMWithCrossStockAttention
    from evaluate_model import evaluate_model, visualize_predictions
...
def train_final_model(
    ...
    # Note: UserWarnings about torch.tensor(sourceTensor) are generally fine for non-leaf tensors
    # or when explicit copying is intended. Using .clone().detach() is more explicit if that's the goal.
    ...
    with mlflow.start_run(run_name=mlflow_run_name) as run:
        ...
        for epoch in range(training_epochs):
            ...
            for batch_sequences, batch_targets in train_loader:
                # DataLoaders should move data to device if tensors were not already on device
                batch_sequences = batch_sequences.to(device) # Explicitly move if not done by DataLoader
        ...
        if test_loader and y_test is not None: # y_test is the original NumPy array
            ...
            mlflow.log_metrics({
                'final_test_loss': final_metrics_on_test.get('test_loss', float('nan')),
                'final_avg_mse_test': final_metrics_on_test.get('avg_mse', float('nan')),
                'final_avg_mape_test': final_metrics_on_test.get('avg_mape', float('nan')),
                'final_avg_direction_accuracy_test': final_metrics_on_test.get('avg_direction_accuracy', float('nan'))
            })
            logger.info("Final model metrics on test set logged to MLflow.")
            if predictions_on_test is not None and y_test is not None:
                visualize_predictions(predictions_on_test, y_test, y_scalers, tickers, plot_output_dir, num_points=20)
                logger.info("Visualizations generated and logged to MLflow artifacts.")
        else:
            logger.info("No test set evaluation performed or y_test was None for visualization.")
...
def run_training(config_path: str, run_id_arg: str) -> Optional[str]:
    ...
    try:
        ...
        plot_output_dir_str = params.get('output_paths', {}).get('training_plots_dir', 'plots/training_plots')
        ...
        if not tickers:
            ...
            feature_data_info = load_processed_features_from_db(db_config, current_dataset_run_id) # Assuming this returns a dict like {'tickers': [...]}
            ...
            try:
                for stock_idx, ticker_name in enumerate(tickers):
                    if test_predictions_np.shape[0] > 0 :
                        ...
                        last_pred_on_test_for_ticker_scaled = test_predictions_np[-1, pred_idx_to_save, stock_idx]
        ...
        logger.error(f"Configuration file not found at {config_path} for run_training, dataset_run_id {run_id_arg}: {e_fnf}", exc_info=True)
        return None
