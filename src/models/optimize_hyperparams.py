...
"""Hyperparameter optimization module for stock price prediction models."""
...
def objective(
    ...
    Args:
        ...
        optimization_epochs: Maximum number of epochs for training
        patience: Early stopping patience

    Returns:
        float: Validation loss (best_val_loss)
    """
    ...
    model_type = trial.suggest_categorical(
        'model_type', ['lstm', 'lstm_attention', 'lstm_cross_attention']
    )
    ...
    # Handle potentially empty X_test, y_test more gracefully
    if X_test.size > 0 and y_test.size > 0:
        ...
        # Create an empty loader for evaluation if test set is empty
        logger.warning(
            "Test data is empty for this Optuna trial. Using a dummy empty loader."
        )
        # Create a dummy empty TensorDataset and DataLoader
        empty_X = torch.empty(
            0, X_train_tensor.shape[1], X_train_tensor.shape[2], 
            X_train_tensor.shape[3], dtype=torch.float32
        )
        empty_y = torch.empty(
            0, y_train_tensor.shape[1], y_train_tensor.shape[2], 
            dtype=torch.float32
        )
    ...
    for epoch in range(num_epochs):
        ...
        if len(test_loader.dataset) > 0:  # type: ignore
            ...
            # If no test data, we can't truly validate. Use train loss instead.
            current_val_loss = avg_train_loss
            logger.warning(
                "No test data available for Optuna trial evaluation, "
                "using average training loss for this epoch."
            )
...
def run_optimization(
    config_path: str, run_id_arg: str
    ...
    Args:
        ...
        run_id_arg: The specific run_id for the scaled dataset to use.
        
    Returns:
        A tuple containing the best parameters and run_id if successful, 
        (None, None) otherwise.
    """
    ...
    try:
        ...
        logger.info(
            f"Using PostgreSQL database at {db_config['host']}:{db_config['port']}"
        )
        
        current_run_id = run_id_arg
        logger.info(
            f"Targeting dataset with run_id: {current_run_id} "
            f"for hyperparameter optimization."
        )
        ...
        patience_optuna = opt_params.get('patience', 5)
        ...
        logger.info(
            f"--- Loading Scaled Data from database for run_id: {current_run_id} ---"
        )
        ...
        if X_train_scaled is None or y_train_scaled is None:
            logger.error(
                f"Failed to load scaled training data (X_train or y_train) "
                f"for run_id: {current_run_id}."
            )
            return None, None
        
        # Ensure X_test_scaled and y_test_scaled are handled if None
        if X_test_scaled is None:
            logger.warning(
                f"X_test_scaled is None for run_id: {current_run_id}. "
                f"Creating empty array."
            )
            # Create empty array with correct dimensions based on X_train_scaled
            X_test_scaled = np.empty(
                (0, X_train_scaled.shape[1], X_train_scaled.shape[2], 
                 X_train_scaled.shape[3]),
                dtype=X_train_scaled.dtype
            )
        if y_test_scaled is None:
            logger.warning(
                f"y_test_scaled is None for run_id: {current_run_id}. "
                f"Creating empty array."
            )
            # Create empty array with correct dimensions
            y_test_scaled = np.empty(
                (0, y_train_scaled.shape[1], y_train_scaled.shape[2]),
                dtype=y_train_scaled.dtype
            )

        logger.info(
            f"X_train_scaled shape: {X_train_scaled.shape}, "
            f"y_train_scaled shape: {y_train_scaled.shape}"
        )
        logger.info(
            f"X_test_scaled shape: {X_test_scaled.shape}, "
            f"y_test_scaled shape: {y_test_scaled.shape}"
        )
        logger.info("--- Finished Loading Scaled Data ---")

        # 2. Load Scalers from database using current_run_id
        logger.info(
            f"--- Loading Scalers from database for run_id: {current_run_id} ---"
        )
        scalers_dict = load_scalers(db_config, current_run_id)
        if scalers_dict is None or 'y_scalers' not in scalers_dict:
            logger.error(
                f"Failed to load scalers or 'y_scalers' not found "
                f"for run_id: {current_run_id}."
            )
            return None, None
        ...
        study.optimize(
            lambda trial: objective(
                ...
                num_features, num_stocks, y_scalers, device, optimization_epochs, 
                patience_optuna
            ), 
            n_trials=n_trials,
            timeout=params['optimization'].get('timeout_seconds', None)
        )
        ...
        logger.info(
            f"--- Saving Best Parameters to database for run_id: {current_run_id} ---"
        )
        ...
        if 'best_params_path' in output_paths_config:
            ...
            try:
                ...
                logger.info(
                    f"--- Also saved Best Parameters to file: "
                    f"{best_params_output_path} ---"
                )
            except Exception as e_file:
                logger.warning(
                    f"Could not save best_params to file {best_params_output_path}: "
                    f"{e_file}"
                )
        ...
        logger.error(
            f"Error in run_optimization for run_id {run_id_arg}: {e}",
            exc_info=True
        )
        return None, None
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description="Hyperparameter optimization script for stock prediction."
    )
    parser.add_argument(
        '--config',
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help='The run_id of the dataset (scaled features, scalers from DB) to use.'
    )
    ...
    logger.info(
        f"Starting optimization script with resolved config: "
    ...
    try:
        ...
        best_params, used_run_id = run_optimization(
            str(config_path_resolved), run_id_arg=cli_run_id_arg
        )
        
        if best_params and used_run_id:
            logger.info(
                f"Optimization completed successfully for run_id: {used_run_id}."
            )
            logger.info(f"Best parameters found: {best_params}")
            print(f"OPTIMIZATION_SUCCESS_RUN_ID:{used_run_id}")
        else:
            logger.error(
                f"Optimization failed for run_id: {cli_run_id_arg}. Check logs."
            )
            sys.exit(1)
        ...
        logger.error(
            f"Fatal error in optimization script for run_id {cli_run_id_arg}: "
            f"{e_main}",
            exc_info=True
        )
        sys.exit(1)