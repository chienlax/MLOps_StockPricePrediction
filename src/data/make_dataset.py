...
def load_data(
    tickers_list: list, period: str, interval: str, fetch_delay: int, db_config: dict
):
    """
    ...
    Args:
        ...
        period (str): The default period to fetch if no data exists for a ticker
                     (e.g., "1y", "max"). This is IGNORED if fetching incrementally.
    ...
    for t in tickers_list:
        ...
        if last_known_date_in_db:
            ...
            if yf_start_date > datetime.now():
                logger.info(
                    f"Calculated start date {yf_start_date_str} for {t} is in the future. "
                    "No data to fetch."
                )
                continue
        ...
        try:
            ...
            if data.empty:
                if yf_start_date_str:
                    date_str = (
                        last_known_date_in_db.strftime("%Y-%m-%d")
                        if last_known_date_in_db
                        else "beginning"
                    )
                    logger.info(f"No new data found for {t} since {date_str}.")
                else:
                    logger.warning(
                        f"No data returned by yfinance for {t} for period '{period}'."
                    )
            ...
            logger.info(
                f"Saved/Updated {records_added} records for {t} in the database."
            )
...
def add_technical_indicators(ticker_data):
    for t in ticker_data:
        ...
        df["MACD"] = ta.trend.MACD(
            df["Close"], window_fast=12, window_sign=9, window_slow=26
        ).macd()
        df["BB_High"] = ta.volatility.BollingerBands(
            df["Close"], window=15, window_dev=2
        ).bollinger_hband()
        ...
        df["Stochastic_D"] = ta.momentum.StochasticOscillator(
            df["High"], df["Low"], df["Close"]
        ).stoch_signal()
        ...
        df["Donchian_High"] = ta.volatility.DonchianChannel(
            df["High"], df["Low"], df["Close"], window=20
        ).donchian_channel_hband()
        df["Donchian_Low"] = ta.volatility.DonchianChannel(
            df["High"], df["Low"], df["Close"], window=20
        ).donchian_channel_lband()
        df["Keltner_High"] = ta.volatility.KeltnerChannel(
            df["High"], df["Low"], df["Close"]
        ).keltner_channel_hband()
        df["Keltner_Low"] = ta.volatility.KeltnerChannel(
            df["High"], df["Low"], df["Close"]
        ...
        df["DI_Positive"] = ta.trend.ADXIndicator(
            df["High"], df["Low"], df["Close"], window=14
        ).adx_pos()
        df["DI_Negative"] = ta.trend.ADXIndicator(
            df["High"], df["Low"], df["Close"], window=14
        ).adx_neg()
...
def preprocess_data(df):
    try:
        ...
        if nan_count > 0:
            logger.warning(
                f"Still have {nan_count} NaN values after fill forward/backward"
            )
        ...
        logger.debug(
            f"Preprocessed data shape changed from {original_shape} to {df.shape}"
        )
        return df
...
def align_and_process_data(ticker_data):
    try:
        ...
        feature_columns = [
            col for col in aligned_data[first_ticker].columns if col != "Target"
        ]
        num_features = len(feature_columns)
        num_stocks = len(tickers)

        logger.info(
            f"Creating 3D array with dimensions: timepoints={len(all_indices)}, "
            f"stocks={num_stocks}, features={num_features}"
        )
        ...
        for i, ticker in enumerate(tickers):
            df = aligned_data[ticker][feature_columns + ["Target"]].ffill().bfill()
        ...
        if nan_count > 0:
            logger.warning(
                f"Found {nan_count} NaN values in processed data before cleaning"
            )

        # Clean any remaining NaNs
        nan_mask = np.isnan(processed_data).any(axis=(1, 2)) | np.isnan(targets).any(
            axis=1
        )
        ...
        logger.info(
            f"Final processed data shape after NaN removal: {processed_data.shape}"
        )
...
def run_processing(config_path: str, mode: str = "full_process") -> Optional[str]:
    ...
    - 'full_process' mode: Calls load_data to ensure raw data is up-to-date,
                           then processes all raw data from DB for training,
                           saves features, and returns a run_id.
    """
    try:
        ...
        logger.info(
            f"Using database configuration: host={db_config['host']}, "
            f"dbname={db_config['dbname']}"
        )

        # Log key configuration parameters
        logger.info(
            f"Processing with parameters: tickers={params['data_loading']['tickers']}, "
        ...
        if mode == "incremental_fetch":
            ...
            load_data(
                tickers_list, default_fetch_period, interval, fetch_delay, db_config
            )
            ...
            logger.info(
                "Step 1.1: Ensuring raw data is up-to-date (calling load_data)..."
            )
            load_data(
                tickers_list, default_fetch_period, interval, fetch_delay, db_config
            )
            ...
            if not ticker_data_from_db or all(
                df.empty for df in ticker_data_from_db.values()
            ):
                logger.error(
                    "No raw data found in the database for any ticker after load_data. "
                    "Cannot proceed."
                )
                return None
            logger.info(
                f"Loaded data for {len(ticker_data_from_db)} tickers from database."
            )
            ...
            for ticker, df_ta in ticker_data_with_ta.items():
                if df_ta.empty:
                    logger.warning(
                        f"DataFrame for {ticker} is empty after TA. "
                        "Skipping preprocessing for it."
                    )
                    continue
                ticker_data_preprocessed[ticker] = preprocess_data(df_ta.copy())

            if not ticker_data_preprocessed or all(
                df.empty for df in ticker_data_preprocessed.values()
            ):
                logger.error(
                    "No data available after preprocessing all tickers. Cannot proceed."
                )
                return None
            ...
            processed_data_np, targets_np, feature_columns_list, final_tickers_list = (
                align_and_process_data(ticker_data_preprocessed.copy())
            )
            # Check if align_and_process_data indicated failure
            if processed_data_np is None or targets_np is None:
                logger.error(
                    "Failed to align and process data into numpy arrays. Cannot proceed."
                )
                return None
            ...
            logger.info(
                f"Step 1.6: Saving processed feature data to database "
                f"with run_id: {current_run_id}..."
            )
            save_processed_features_to_db(
                db_config,
                processed_data_np,
            ...
            logger.info(
                f"Step 1.6: Successfully saved processed feature data "
            ...
            logger.error(
                ...
                "Choose 'incremental_fetch' or 'full_process'."
            )
            return None
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Data ingestion and processing script for stock prediction."
    )
    parser.add_argument(
        "--config",
        type=str,
    ...
    if not config_path_resolved.is_absolute():
        if (Path.cwd() / config_path_resolved).exists():
            ...
            config_path_resolved = (
                Path(__file__).parent.parent.parent / config_path_resolved
            ).resolve()
        else:
            logger.error(
                f"Configuration file not found: {config_path_arg} "
                f"(resolved to {config_path_resolved})"
            )
            sys.exit(1)
    ...
    logger.info(
        f"Starting data processing script with resolved config: "
        f"{config_path_resolved} in mode: {mode_arg}"
    )
    ...
    try:
        with open(config_path_resolved, "r") as f:
            ...
            missing_fields = [
                ...
                if field not in config_params["database"]
            ]
            if missing_fields:
                logger.error(
        ...
        logger.info(
            f"Using PostgreSQL database at "
            f"{config_params['database']['host']}:{config_params['database']['port']}"
        )
        ...
        if mode_arg == "full_process":
            if returned_value:
                logger.info(
                    f"Full processing completed. Dataset run_id: {returned_value}"
                )
                print(f"RUN_ID:{returned_value}")
            else:
                logger.error(
                    "Full processing mode did not return a run_id. Check logs for errors."
                )
                sys.exit(1)
        else:
            logger.info("Incremental fetch mode completed.")

        logger.info(f"Data processing script (mode: {mode_arg}) finished successfully.")

    except yaml.YAMLError as e_yaml:
        logger.error(
            f"Error parsing configuration file {config_path_resolved}: {e_yaml}",
            exc_info=True,
        )
        sys.exit(1)
    except Exception as e_main_script:
        logger.error(
            f"Fatal error in data processing script (mode: {mode_arg}): {e_main_script}",
            exc_info=True,
        )
        sys.exit(1)
