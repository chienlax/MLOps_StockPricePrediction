...
def load_data(
    ...
    for t in tickers_list:
        ...
        try:
            ...
            if data.empty:
                if yf_start_date_str:
                    date_str = (
                        ...
                        if last_known_date_in_db else 'beginning'
                    )
            ...
            logger.error(
                f"Error processing ticker {t}: {e_ticker}", exc_info=True
            )
...
def add_technical_indicators(ticker_data):
    for t in ticker_data:
        ...
        df['Price_Rate_Of_Change'] = ta.momentum.ROCIndicator(
            df['Close'], 12
        ).roc()
...
def align_and_process_data(ticker_data):
    try:
        ...
        for t in tickers:
            aligned_data[t] = ticker_data[t].reindex(
                index=all_indices
            ).sort_index()
        
        # Get feature columns (excluding Target)
        first_ticker = tickers[0]
        feature_columns = [col for col in aligned_data[first_ticker].columns 
                          if col != 'Target']
