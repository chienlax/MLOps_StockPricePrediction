...
def scale_data(X_train, X_test, y_train, y_test, num_features, num_stocks):
    ...
    for stock_idx in range(num_stocks):
        for feature_idx in range(num_features):
            ...
            X_train_scaled[:, :, stock_idx, feature_idx] = scalers_x[stock_idx][
                feature_idx].transform(
                X_train[:, :, stock_idx, feature_idx].reshape(-1, 1)
            ).reshape(X_train.shape[0], -1)
            
            # Only transform X_test if it's not empty
            if X_test.size > 0:
                X_test_scaled[:, :, stock_idx, feature_idx] = scalers_x[stock_idx][
                    feature_idx].transform(
                    X_test[:, :, stock_idx, feature_idx].reshape(-1, 1)
    ...
    for stock_idx in range(num_stocks):
        ...
        y_train_scaled[:, 0, stock_idx] = y_scalers[stock_idx].transform(
            y_train_for_stock
        ).flatten()
        ...
        if y_test.size > 0:
            y_test_scaled[:, 0, stock_idx] = y_scalers[stock_idx].transform(
                y_test[:, 0, stock_idx].reshape(-1, 1)
            ).flatten()
...
def run_feature_building(config_path: str, run_id_arg: str) -> Optional[str]:
    ...
    try:
        ...
        if (processed_data_np is None or targets_np is None or 
                processed_data_np.size == 0 or targets_np.size == 0):
            logger.error(
        ...
        X_train_scaled, X_test_scaled, y_train_scaled, y_test_scaled, scalers_x, y_scalers = (
            scale_data(
        ...
        logger.info(
            f"--- Saving Scalers to database for run_id: {current_run_id} ---"
        )
        scalers_dict = {
            'scalers_x': scalers_x,
            'y_scalers': y_scalers,
        ...
        logger.error(
            f"Error in run_feature_building for run_id {run_id_arg}: {e}",
            exc_info=True
        )
        return None
