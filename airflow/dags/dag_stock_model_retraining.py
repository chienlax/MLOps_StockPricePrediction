...
def callable_run_make_dataset_full_retrain(**kwargs):
    ...
    command = [
        "python", script_path, "--config", config_file_path, "--mode", "full_process"
    ]
    ...
    if process.returncode != 0:
        ...
        raise Exception(
            f"Retraining DAG: make_dataset.py failed. Error: {stderr}"
        )
...
def callable_train_candidate_and_get_ids(**kwargs):
    ...
    new_dataset_run_id = ti.xcom_pull(
        task_ids='process_all_data_for_retraining_task', 
    ...
    mlflow_tracking_uri = os.environ.get(
        'MLFLOW_TRACKING_URI', 
        params['mlflow'].get('tracking_uri')
    )
    
    command = [
        "python", str(TRAIN_SCRIPT), "--config", str(CONFIG_PATH), 
        "--run_id", new_dataset_run_id
    ]
    env = os.environ.copy()
    ...
    ti.xcom_push(
        key='candidate_model_mlflow_run_id', 
        value=candidate_model_mlflow_run_id
    )
    ti.xcom_push(
        key='candidate_training_dataset_run_id', 
        value=new_dataset_run_id
...
def callable_evaluate_and_branch_promotion(**kwargs):
    ...
    candidate_mlflow_run_id = ti.xcom_pull(
        task_ids='train_candidate_model_task', 
        key='candidate_model_mlflow_run_id'
    )
    ...
    if is_candidate_better:
        ...
        return params_glob_retrain['airflow_dags'].get(
            'promotion_task_id', 
            'promote_candidate_to_production_task'
        )
    else:
...
with DAG(
    ...
    common_env_vars_retrain = {
        ...
        'MLFLOW_TRACKING_URI': os.environ.get(
            'MLFLOW_TRACKING_URI',
            params_glob_retrain['mlflow'].get('tracking_uri')
        )
    }
