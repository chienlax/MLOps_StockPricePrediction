...
try:
    ...
    log.error(
        f"CRITICAL: Could not load params.yaml at {CONFIG_PATH} "
        f"for Retraining DAG. Error: {e}"
    )
    params_glob_retrain = {
        "airflow_dags": {
        ...
        "mlflow": {
            ...
            "experiment_name": "Fallback_Experiment",
        },
...
def callable_initialize_database_for_retrain(**kwargs):
    """Initialize database connection for retraining pipeline."""
    import yaml
...
def callable_run_make_dataset_full_retrain(**kwargs):
    """Run data processing pipeline and return dataset run ID."""
    ti = kwargs["ti"]
...
def callable_train_candidate_and_get_ids(**kwargs):
    """Train a candidate model and capture its MLflow run ID."""
    ti = kwargs["ti"]
    new_dataset_run_id = ti.xcom_pull(
        task_ids="process_all_data_for_retraining_task", key="new_dataset_run_id"
    )
    ...
    mlflow_tracking_uri = os.environ.get(
        "MLFLOW_TRACKING_URI", params["mlflow"].get("tracking_uri")
    )
    ...
    mlflow_run_id_line = [
        ...
        if line.startswith("TRAINING_SUCCESS_MLFLOW_RUN_ID:")
    ]
    if not mlflow_run_id_line:
        raise Exception(
            "Retraining DAG: train_model.py did not output "
            "TRAINING_SUCCESS_MLFLOW_RUN_ID."
        )

    candidate_model_mlflow_run_id = (
        mlflow_run_id_line[-1].split("TRAINING_SUCCESS_MLFLOW_RUN_ID:")[1].strip()
    )
    log.info(
        f"Retraining DAG: Candidate model trained. "
        f"MLflow Run ID: {candidate_model_mlflow_run_id}"
    )

    ti.xcom_push(
        key="candidate_model_mlflow_run_id", value=candidate_model_mlflow_run_id
    )
    ti.xcom_push(
        key="candidate_training_dataset_run_id", value=new_dataset_run_id
    )  # For evaluation context

    return candidate_model_mlflow_run_id


def callable_evaluate_and_branch_promotion(**kwargs):
    """Compare candidate model to production model and decide on promotion."""
    ti = kwargs["ti"]
    candidate_mlflow_run_id = ti.xcom_pull(
        task_ids="train_candidate_model_task", key="candidate_model_mlflow_run_id"
    )
    candidate_dataset_run_id = ti.xcom_pull(
        task_ids="train_candidate_model_task", key="candidate_training_dataset_run_id"
    )

    if not candidate_mlflow_run_id or not candidate_dataset_run_id:
        log.error(
            "Missing candidate_mlflow_run_id or candidate_dataset_run_id "
            "for evaluation."
        )
        return params_glob_retrain["airflow_dags"].get(
            "no_promotion_task_id", "do_not_promote_task"
        )  # Default branch
    ...
    # Metric from train_model.py's test eval
    eval_metric_name = promotion_cfg.get("metric", "final_avg_mape_test")
    higher_is_better = promotion_cfg.get(
    ...
    if candidate_metric_value is None:
        log.warning(
            f"Metric '{eval_metric_name}' not found for candidate model "
            f"{candidate_mlflow_run_id}. Cannot compare."
        )
        return params_glob_retrain["airflow_dags"].get(
            "no_promotion_task_id", "do_not_promote_task"
        )
    ...
    log.info(
        f"Candidate model ({candidate_mlflow_run_id}) evaluation metric "
        f"({eval_metric_name}): {candidate_metric_value}"
    )

    prod_model_metric_value = None
    current_prod_alias = "Production"
    try:
        prod_version_obj = client.get_model_version_by_alias(
            name=registered_model_name, alias=current_prod_alias
        )
        ...
        log.info(
            f"Current Production model ({prod_mlflow_run_id}) metric "
            f"({eval_metric_name}): {prod_model_metric_value}"
        )
    except (
        mlflow.exceptions.MlflowException
    ) as e:  # Handles case where alias or model doesn't exist
        log.info(
            f"No current Production model or its metric found to compare against "
            f"({e}). Candidate will be promoted if valid."
        )
    ...
    if prod_model_metric_value is not None:
        if higher_is_better:
            ...
            is_candidate_better = candidate_metric_value < prod_model_metric_value
    elif candidate_metric_value is not None:  # No production model to compare
        is_candidate_better = True

    if is_candidate_better:
        log.info(
            "Candidate model is better or no production model to compare. "
            "Proceeding with promotion logic."
        )
        ti.xcom_push(
            key="candidate_model_version_to_promote_run_id",
            value=candidate_mlflow_run_id,
        )  # Pass run_id for promotion task
        return params_glob_retrain["airflow_dags"].get(
            "promotion_task_id", "promote_candidate_to_production_task"
        )
    else:
...
def callable_promote_model_to_production(**kwargs):
    """Promote the candidate model to production by setting the alias."""
    ti = kwargs["ti"]
    candidate_mlflow_run_id = ti.xcom_pull(
        task_ids="evaluate_and_branch_on_promotion_decision",
        key="candidate_model_version_to_promote_run_id",
    )
    ...
    if not versions:
        log.error(
            f"No model version found in registry for run_id "
            f"{candidate_mlflow_run_id}. Cannot promote."
        )
        return

    candidate_version_to_promote = sorted(
        versions, key=lambda v: int(v.version), reverse=True
    )[0].version

    log.info(
        f"Promoting model '{registered_model_name}' version "
        f"{candidate_version_to_promote} (from run {candidate_mlflow_run_id}) "
        f"to alias '{production_alias_name}'."
    )
    ...
    log.info(
        ...
        f"{candidate_version_to_promote} of model '{registered_model_name}'."
    )
...
with DAG(
    ...
    common_env_vars_retrain = {
        ...
        "MLFLOW_TRACKING_URI": os.environ.get(
            "MLFLOW_TRACKING_URI", params_glob_retrain["mlflow"].get("tracking_uri")
        ),
    }
    ...
    xcom_new_dataset_run_id = (
        "{{ ti.xcom_pull(task_ids='process_all_data_for_retraining_task', "
        "key='new_dataset_run_id') }}"
    )

    task_build_features = BashOperator(
        task_id="build_features_for_retraining",
        bash_command=(
            f"python {BUILD_FEATURES_SCRIPT} "
            f"--config {CONFIG_PATH} "
            f'--run_id "{xcom_new_dataset_run_id}"'
        ),
    ...
    promotion_task_id_from_cfg = params_glob_retrain["airflow_dags"].get(
        "promotion_task_id", "promote_candidate_to_production_task"
    )
    no_promotion_task_id_from_cfg = params_glob_retrain["airflow_dags"].get(
        "no_promotion_task_id", "do_not_promote_task"
    )
