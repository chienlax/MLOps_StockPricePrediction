...
# Unique temp dir for this DAG
PREDICTION_INPUT_TEMP_DIR = "/tmp/prod_daily_pred_inputs"
...
def callable_get_production_model_details_daily(**kwargs):
    ...
    if not mlflow_tracking_uri or not base_registered_model_name:
        raise ValueError(
            "MLflow tracking URI or base_registered_model_name not configured."
        )
    ...
    try:
        log.info(
            f"Querying MLflow for model '{base_registered_model_name}' "
            f"with alias '{production_alias}'..."
        )
        model_version_obj = client.get_model_version_by_alias(
            name=base_registered_model_name, alias=production_alias
        )

        production_model_uri_for_loading = (
            f"models:/{base_registered_model_name}@{production_alias}"
        )
        production_version_number_str = model_version_obj.version
        prod_model_origin_mlflow_run_id = model_version_obj.run_id

        log.info(
            f"Daily Ops: Using Prod Model: "
            f"URI='{production_model_uri_for_loading}', "
            f"Ver='{production_version_number_str}', "
            f"Training MLflow Run='{prod_model_origin_mlflow_run_id}'"
        )

        mlflow_run_details = client.get_run(prod_model_origin_mlflow_run_id)
        prod_model_training_dataset_run_id = mlflow_run_details.data.params.get(
            "dataset_run_id"
        )

        if not prod_model_training_dataset_run_id:
            raise ValueError(
                f"'dataset_run_id' param missing from Prod Model's "
                f"(MLflow Run {prod_model_origin_mlflow_run_id}) params."
            )
        log.info(
            f"Daily Ops: Prod model's training dataset_run_id: "
            f"{prod_model_training_dataset_run_id}"
        )

    except mlflow.exceptions.MlflowException as e:
        if "RESOURCE_DOES_NOT_EXIST" in str(e) and (
            f"alias '{production_alias}'" in str(e)
            or f"model '{base_registered_model_name}'" in str(e)
        ):
            log.error(
                f"CRITICAL: Alias '{production_alias}' or model "
                f"'{base_registered_model_name}' does not exist."
            )
            raise ValueError(
                f"Production alias/model not found. Please ensure "
                f"'{base_registered_model_name}' has a version "
                f"aliased as '{production_alias}'."
            )
        else:
        ...
        log.error(
            f"Unexpected error in callable_get_production_model_details: {e}",
            exc_info=True,
        )
        raise

    ti.xcom_push(
        key="production_model_uri_for_loading", value=production_model_uri_for_loading
    )
    ti.xcom_push(key="production_model_base_name", value=base_registered_model_name)
    ti.xcom_push(
        key="production_model_version_number", value=production_version_number_str
    )
    ti.xcom_push(
        key="production_model_training_dataset_run_id",
        value=prod_model_training_dataset_run_id,
    )
    return "Production model details fetched."


def callable_branch_on_monitoring_result_daily(**kwargs):
    ti = kwargs["ti"]
    monitoring_output = ti.xcom_pull(
        task_ids="monitor_model_performance_daily_task", key="return_value"
    )
    log.info(
        f"Daily Ops DAG: Monitoring script output for branching: "
        f"{monitoring_output}"
    )
    ...
    retraining_branch_task_id = airflow_dag_cfg.get(
        "retraining_trigger_task_id", "trigger_retraining_pipeline_task"
    )
    no_retraining_branch_task_id = airflow_dag_cfg.get(
        "no_retraining_task_id", "no_retraining_needed_task"
    )

    if (
        monitoring_output
        and f"NEXT_TASK_ID:{retraining_branch_task_id}" in monitoring_output
    ):
        log.info(
            f"Branching decision: proceed to task_id '{retraining_branch_task_id}'"
        )
        return retraining_branch_task_id
    elif (
        monitoring_output
        and f"NEXT_TASK_ID:{no_retraining_branch_task_id}" in monitoring_output
    ):
        log.info(
            f"Branching decision: proceed to task_id '{no_retraining_branch_task_id}'"
        )
        return no_retraining_branch_task_id
    else:
        log.warning(
            f"Could not determine next task from monitoring output: "
            f"'{monitoring_output}'. Defaulting to '{no_retraining_branch_task_id}'."
        )
        return no_retraining_branch_task_id
...
default_args_daily = {
    ...
    "email": [params_glob.get("alert_email", "alert@example.com")],
...
with DAG(
    ...
    schedule=params_glob.get("schedules", {}).get("daily_dag", "0 2 * * 1-5"),
    ...
    common_env_vars_daily = {
        ...
        "MLFLOW_TRACKING_URI": os.environ.get(
            "MLFLOW_TRACKING_URI", params_glob["mlflow"].get("tracking_uri")
        ),
    }

    task_init_db = PythonOperator(
        task_id="initialize_database_daily",
    ...
    task_get_prod_model = PythonOperator(
        task_id="get_production_model_details_daily",
        python_callable=callable_get_production_model_details_daily,
    )

    xcom_prod_training_dataset_run_id = (
        "{{ ti.xcom_pull(task_ids='get_production_model_details_daily', "
        "key='production_model_training_dataset_run_id') }}"
    )
    xcom_prod_model_uri_for_loading = (
        "{{ ti.xcom_pull(task_ids='get_production_model_details_daily', "
        "key='production_model_uri_for_loading') }}"
    )
    xcom_prod_model_base_name = (
        "{{ ti.xcom_pull(task_ids='get_production_model_details_daily', "
        "key='production_model_base_name') }}"
    )
    xcom_prod_model_version_number = (
        "{{ ti.xcom_pull(task_ids='get_production_model_details_daily', "
        "key='production_model_version_number') }}"
    )
    ...
    xcom_input_seq_file_path = (
        "{{ ti.xcom_pull(task_ids='prepare_daily_prediction_input')"
    ...
    task_monitor_performance = BashOperator(
        task_id="monitor_model_performance_daily_task",
        bash_command=(
            f"python {MONITOR_PERFORMANCE_SCRIPT} " f"--config {CONFIG_PATH}"
        ),
        env=common_env_vars_daily,
        do_xcom_push=True,
    )
    ...
    branch_on_monitor = BranchPythonOperator(
        task_id="branch_based_on_performance",
    ...
    retraining_dag_to_trigger = params_glob["airflow_dags"][
        "retraining_pipeline_dag_id"
    ]
