...
def callable_get_production_model_details_daily(**kwargs):
    ...
    try:
        ...
        if ("RESOURCE_DOES_NOT_EXIST" in str(e) and 
                (f"alias '{production_alias}'" in str(e) or 
                 f"model '{base_registered_model_name}'" in str(e))):
            log.error(
            ...
            log.error(
                f"Error querying MLflow for production model: {e}", 
                exc_info=True
            )
            raise
    except Exception as e:
    ...
    ti.xcom_push(
        ...
        value=production_model_uri_for_loading
    )
    ti.xcom_push(
        key='production_model_base_name', 
        value=base_registered_model_name
    )
    ti.xcom_push(
        key='production_model_version_number', 
        value=production_version_number_str
    )
    ti.xcom_push(
...
def callable_branch_on_monitoring_result_daily(**kwargs):
    ...
    monitoring_output = ti.xcom_pull(
        task_ids='monitor_model_performance_daily_task', 
        key='return_value'
    )
    log.info(
    ...
    retraining_branch_task_id = airflow_dag_cfg.get(
        'retraining_trigger_task_id', 
        'trigger_retraining_pipeline_task'
    )
    no_retraining_branch_task_id = airflow_dag_cfg.get(
        'no_retraining_task_id', 
        'no_retraining_needed_task'
    )

    if (monitoring_output and 
            f"NEXT_TASK_ID:{retraining_branch_task_id}" in monitoring_output):
        log.info(f"Branching decision: proceed to task_id '{retraining_branch_task_id}'")
        return retraining_branch_task_id
    elif (monitoring_output and 
            f"NEXT_TASK_ID:{no_retraining_branch_task_id}" in monitoring_output):
...
with DAG(
    ...
    common_env_vars_daily = {
        ...
        'MLFLOW_TRACKING_URI': os.environ.get(
            'MLFLOW_TRACKING_URI', 
            params_glob['mlflow'].get('tracking_uri')
        )
    }
