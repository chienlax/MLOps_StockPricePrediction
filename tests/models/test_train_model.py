...
# from models.model_definitions import StockLSTM # If needed for spec
# from models.evaluate_model import evaluate_model, visualize_predictions # For mocking
...
def mock_training_params_cfg_train():
    return {'epochs': 2} # Small number for tests
...
@pytest.fixture
def sample_scaled_data_train(): # Similar to _opt, but can be distinct if needed
    return {
        'X_train': torch.rand(20, 5, 2, 3), # (samples, seq_len, stocks, features)
        'y_train': torch.rand(20, 1, 2),   # (samples, pred_len, stocks)
        'X_test': torch.rand(10, 5, 2, 3),
        'y_test': torch.rand(10, 1, 2),
    }

@pytest.fixture
def mock_y_scalers_train(): # Similar to _opt
...
@pytest.fixture # Added for device
def device_eval():
    return torch.device('cpu')


# --- Tests for train_final_model ---
# COMPLEXITY NOTICE: Testing 'train_final_model' thoroughly is hard due to the training loop.
# We will mock the model, data loader, evaluation, and MLflow interactions.
class TestTrainFinalModel:
    @patch('models.train_model.StockLSTM') # Mock the actual model class
    ...
    @patch('models.train_model.mlflow') # Mock the entire mlflow module
    def test_train_final_model_flow_and_mlflow_calls(
            ...
            mock_y_scalers_train, mock_tickers_train, device_eval, # Ensure device_eval is here
        ...
        mock_dataloader.return_value = [(dummy_X_batch, dummy_y_batch)]
        ...
        model_obj, preds_test, metrics_test, mlflow_run_id_out = train_final_model(
            ...
            sample_scaled_data_train['X_test'], sample_scaled_data_train['y_test'],
        ...
        mock_viz_preds.assert_called_once_with(
            pytest.ANY, sample_scaled_data_train['y_test'], mock_y_scalers_train,
    ...
    def test_train_final_model_no_test_set(self, mock_mlflow, mock_dataloader, mock_lstm_basic,
                                           sample_best_hyperparams_train, sample_scaled_data_train,
                                           mock_y_scalers_train, mock_tickers_train, device_eval,
                                           mock_training_params_cfg_train, mock_mlflow_config_train,
                                           tmp_path, caplog):
        mock_model_inst = MagicMock(spec=nn.Module); mock_model_inst.to.return_value = mock_model_inst
        mock_model_inst.return_value = torch.rand(32,1,2); mock_lstm_basic.return_value = mock_model_inst
        mock_dataloader.return_value = [(torch.rand(32,5,2,3), torch.rand(32,1,2))]
        mock_mlflow_run = MagicMock(); mock_mlflow_run.info.run_id = "mfr_no_test"
        mock_mlflow.start_run.return_value.__enter__.return_value = mock_mlflow_run
...
class TestRunTraining:
    @patch('models.train_model.yaml.safe_load') # Patch yaml.safe_load
    ...
    def test_run_training_success(self, mock_save_pred, mock_mlflow_module, mock_train_final,
        ...
        mock_yaml_safe_load.return_value = mock_params_config_train # <--- Use the patched mock
        mock_load_scaled.side_effect = lambda db, drun_id, set_name: getattr(sample_scaled_data_train[set_name], 'numpy', lambda: sample_scaled_data_train[set_name])() if isinstance(sample_scaled_data_train[set_name], torch.Tensor) else sample_scaled_data_train[set_name]
        ...
        mock_mlflow_client_inst = MagicMock()
        ...
        mock_mlflow_client_inst.get_latest_versions.return_value = []
    ...
    @patch('models.train_model.load_scaled_features')
    def test_run_training_load_scaled_fails(self, mock_load_scaled, mock_yaml_safe_load,
        ...
        result = run_training(str(tmp_path/"cfg.yaml"), dataset_run_id)
    ...
    @patch('models.train_model.load_scaled_features')
    @patch('models.train_model.load_scalers')
    def test_run_training_load_scalers_fails(self, mock_load_scalers, mock_load_scaled,
                                             mock_yaml_safe_load, mock_params_config_train,
                                             sample_scaled_data_train, tmp_path, caplog):
        dataset_run_id = "train_fail_lsc"
        mock_yaml_safe_load.return_value = mock_params_config_train
        mock_load_scaled.side_effect = lambda db, dr_id, set_name: sample_scaled_data_train[set_name].numpy()
    ...
    @patch('models.train_model.load_scalers')
    @patch('models.train_model.load_optimization_results')
    def test_run_training_load_opt_res_fails(self, mock_load_opt_res, mock_load_scalers, mock_load_scaled,
                                             mock_yaml_safe_load, mock_params_config_train,
    ...
    def test_run_training_promotion_fails_no_version(self, mock_mlflow_module, mock_train_final,
        ...
        config_file = tmp_path / "params_train_promo_fail.yaml" # <--- Defined config_file
        ...
        result_mlflow_run_id = run_training(str(config_file), dataset_run_id) # <--- Used config_file
